...
\caption{\label{tab:vole-table}Coefficient table of fit linear model of vole data.}
\centering
...
Level & t & p\\
\hline
vitamin\_C & -2.113 & 0.037\\
\hline
vitamin\_E & -1.713 & 0.090\\
\hline
\end{tabular}
...
Level & t & p\\
\hline
vitamin\_C & -1.981 & 0.051\\
\hline
vitamin\_E & -1.628 & 0.108\\
\hline
\end{tabular}
...
\caption{\label{tab:unnamed-chunk-55}Coefficients of the model that includes the covariate age.}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & Estimate & Std. Error & t value & Pr(>|t|)\\
...
Regression to the mean happens when \(\rho < 1\) and it's consequences
increase as \(\rho\) goes to zero. What is meant by ``consequences''?

If the fat weight of the \emph{ob/ob} group at baseline is 1.2 standard
deviations smaller than that of the \emph{+/+} group, and if \(\rho=0\),
then we'd expect the difference at the end to be zero. To get to zero
difference at the end, the \emph{ob/ob} mice would have to gain more fat
weight than the \emph{+/+} mice. Since the expectation of the mean
difference at the end is zero the expectation of the change score
\emph{is bigger for the ob/ob mice than for the +/+ mice}. That is the
expectation of the \emph{difference} in change score is conditional on
(or ``a function of'') the difference in fat weight at baseline.
...
\section{Best practices xxx: Use a covariate instead of normalizing a
response}\label{best-practices-xxx-use-a-covariate-instead-of-normalizing-a-response}
