...
\caption{\label{tab:unnamed-chunk-7}Root mass for three treatment levels in wide format}
\centering
...
\caption{\label{tab:unnamed-chunk-8}Root mass for three treatment levels in long format}
\centering
\begin{tabular}[t]{l|r}
\hline
treatment & root\_mass\\
\hline
control & 10.18\\
\hline
control & 10.40\\
...
\hline
fish\_ID & treatment & length & pre\_1 & pre\_2 & pre\_3 & post\_1 & post\_2 & post\_3\\
\hline
...
\hline
fish\_ID & treatment & length & Order & sprint\_pre & sprint\_post\\
\hline
...
lifespan changes by -89.9166667 days, relative to the control.
...
The estimates in the coefficient table are the estimates of the
parameters in this linear model. The estimates are the coefficients of
the \textbf{fit model}, which is
...
to understand this plot. Be able to ``visualize'' the meaning of the
...
regression model? What are the numeric values of \(vitamin_C\) and
\(vitamin_E\)? The answer is very clever: \(vitamin\_E\) and
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{vole[, treatment_reorder}\OperatorTok{:}\ErrorTok{=}\KeywordTok{factor}\NormalTok{(treatment, }
...
any further analysis. One would typically replace the original variable
with the re-ordered levels instead of creating a new variable as here,
but the new variable was created to avoid bugs with the model fit object
``fit'' (the level order in the data and in the model object need to be
the same). Regardless, Refit the model to see how this re-ordering
changes the coefficients

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fit2 <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(lifespan }\OperatorTok{~}\StringTok{ }\NormalTok{treatment_reorder, }\DataTypeTok{data=}\NormalTok{vole)}
...
## treatment_reordervitamin_E  25.1604   64.94462 0.3874132 6.993356e-01
## treatment_reordercontrol   115.0771   54.45772 2.1131453 3.726632e-02
\end{verbatim}
