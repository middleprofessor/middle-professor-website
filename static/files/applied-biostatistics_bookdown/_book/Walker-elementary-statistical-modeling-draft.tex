...
something called the model matrix, which we'll cover in the next
chapter. You won't see these columns in your data. But if you did, it
would look something like this

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dt <-}\StringTok{ }\NormalTok{vole[, .SD, .SDcols=}\KeywordTok{c}\NormalTok{(}\StringTok{"lifespan"}\NormalTok{, }\StringTok{"treatment"}\NormalTok{)]}
\NormalTok{dt <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(dt[treatment}\OperatorTok{==}\StringTok{"control"}\NormalTok{][}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{,],}
\NormalTok{            dt[treatment}\OperatorTok{==}\StringTok{"vitamin_E"}\NormalTok{][}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{,],}
\NormalTok{            dt[treatment}\OperatorTok{==}\StringTok{"vitamin_C"}\NormalTok{][}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{,]}
\NormalTok{            )}
\NormalTok{dt[, vitamin_E}\OperatorTok{:}\ErrorTok{=}\KeywordTok{ifelse}\NormalTok{(treatment}\OperatorTok{==}\StringTok{"vitamin_E"}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{)]}
\NormalTok{dt[, vitamin_C}\OperatorTok{:}\ErrorTok{=}\KeywordTok{ifelse}\NormalTok{(treatment}\OperatorTok{==}\StringTok{"vitamin_C"}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{)]}
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(dt)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  method=``revpairwise''. Contrast will create various differences. Here
  I've specified all pairwise differences (the ``rev'' reverses the
  order of the subtraction). Notice that the statistics (estimate, SE,
  etc) are equal to the same statistics for \(b_1\) and \(b_2\) of the
  linear model. I said earlier that these coefficients are contrasts!
\item
  adjust=``none''. In classical frequetist hypothesis testing, the
  ...
  is important, and we will retur to this in a future chapter, but here
...
\item
  ...
  put in the table. The first value specifies if to include the CI or
  not, the second value specifies to include the \(t\) and \(p\)-value
  or not.
\end{enumerate}
...
\section{Inference statistics}\label{inference-statistics}
...
\section{Inference assumptions}\label{inference-assumptions}

\subsection{normal distribution of
error}\label{normal-distribution-of-error}

\subsubsection{extending the model if not
normal}\label{extending-the-model-if-not-normal}

\subsection{independence of error}\label{independence-of-error}

\subsubsection{extending the model if not
independent}\label{extending-the-model-if-not-independent}

\subsection{identical distribution (homogeneity) of
error}\label{identical-distribution-homogeneity-of-error}

\subsubsection{extending the model if not
homogenous}\label{extending-the-model-if-not-homogenous}

\section{Classical inference}\label{classical-inference}
