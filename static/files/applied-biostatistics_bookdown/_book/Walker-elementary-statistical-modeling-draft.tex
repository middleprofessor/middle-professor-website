...
standard deviation sigma''). This line explicitly specifies the
...
generated. The linear model way ``generates'' data by constructing
\(y_i\) from the \(x_i\) and then adding random, normal error. The
statistical model way ``generates'' data by simply drawing a random
variable from a normal distribution with a specified mean and variance.
The statistical model way explicitly defines all parameters, including
the parameters of the linear predictor (\(\beta_0\) and \(\beta_1\)),
the conditional mean \(\mu\) and the variance \(\sigma\). The linear
model way only defines the parameters of the linear predictor, and often
these are referrred to as ``the parameters'' in the sense that there are
not other parameters. Because of the explicit statement of all
parameters, the statistical model way to define the linear model is more
useful in that it is easily generalized to more complex models,
including hierarchical models, generalized linear models, and Bayesian
models. This is why I call it the ``statistical model'' way of
specification.
...
standard deviation of the residuals of the model
\(\sqrt{\frac{ss_{residual}}{df_{residual}}}\).
...
distributed, not the residuals from the fit model. Some other
consequences of thinking about this way are: First, if we set the mean
of the Poisson ``error'' to zero (like with a normal distribution), then
the error term for every observation would \emph{have to} be zero
(because the only way to get a mean of zero with non-negative integers
is if every value is zero). Second, if the study is modeling the effect
of a treatment on the counts (that is, the \(X\) are dummy variables)
then \(\beta_0\) is the expected mean count of the control (or
reference) group. But if we add non-zero Poisson error to this, then the
mean of the control group would be larger than \(\beta_0\). This doesn't
make sense. And finally, equation \eqref{eq:spec1-poisson} generates a
continuous response, instead of an integer, because \(\beta_0\) and
\(\beta_1\) are continuous.

A better way to think about the data generation for a linear model,
because this naturally leads to the way to think about data generation
...
\section{Block and repeated measures
effects}\label{block-and-repeated-measures-effects}
...
The plots in experiment 1, individuals in experiment 2, mice in
experiment 3, plots in experiment 4, nests in experiment 5, cultures in
experiment 6, and tanks in experiment 7 are the experimental units,
meaning that it is at this level that the experimenter is controlling
the conditions. There is variation among treatments due to treatment
effects, variation within treatments due to sampling, and variation
within units due to a shared or common environments. This within batch
variation is due to \textbf{block} effects. The
plots/individuals/mice/nests/cultures/tanks are often referred to as
\textbf{blocks}. The multiple measures within a block are often called
\textbf{repeated measures}, especially if the block is an experimental
animal such as a mouse or human. If multiple measures within a treatment
level within a block (that is, within a \(block \times treatment\)
combination) are taken over time, the data are \textbf{longitudinal}.
Sometimes in cell biology, the subsamples within a treatment within a
block are called ``replicates'', as they are replicates of this
...
\section{Block (repeated measures) effects in statistical
models}\label{block-repeated-measures-effects-in-statistical-models}
...
among blocks. In the above equation, I've used parentheses to show how
these combine into the random intercept that is unique for each block.
Similarly, the random slope has a fixed part (\(\beta_1\)) that is
common to all observations and a random component (\(\beta_{1j}\)) that
is common within a block but differs among blocks. Again, these are
collected within a pair of parentheses in the equation above.
...
\textbf{NHST blues} -- A paired \emph{t}-test is equivalent to the
special case of the linear mixed model when there is a single factor
with two treatment levels and \(n=1\) measure for each treatment level
within each block. A linear mixed model is much more flexible than a
...
and it can better handle missing data.

Here is a simple simulation of a design that is similar to experiment 3
with a single factor with two treatment levels and both levels applied
to the same experimental unit.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n <-}\StringTok{ }\DecValTok{10} \CommentTok{# number of blocks}
\NormalTok{x <-}\StringTok{ }\KeywordTok{rep}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"Cn"}\NormalTok{,}\StringTok{"T+"}\NormalTok{), }\DataTypeTok{each=}\NormalTok{n) }\CommentTok{# treatments}
...
## (Intercept) 10.274104 0.3632787  9 28.281608 4.201969e-10
## XT+          1.057625 0.4678365  9  2.260673 5.012143e-02
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# now delete two random observations from the treated level}
\NormalTok{NA_rows <-}\StringTok{ }\KeywordTok{sample}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{n, }\DataTypeTok{size=}\DecValTok{2}\NormalTok{)}
\NormalTok{fake_data[, Y2}\OperatorTok{:}\ErrorTok{=}\NormalTok{Y] }\CommentTok{# copy Y column}
\NormalTok{fake_data[(NA_rows}\OperatorTok{+}\DecValTok{10}\NormalTok{), Y2}\OperatorTok{:}\ErrorTok{=}\OtherTok{NA}\NormalTok{] }\CommentTok{# replace the two obs in Y2 with NA}
\KeywordTok{coef}\NormalTok{(}\KeywordTok{summary}\NormalTok{(}\KeywordTok{lme}\NormalTok{(Y2}\OperatorTok{~}\NormalTok{X, }\DataTypeTok{random =} \OperatorTok{~}\DecValTok{1}\OperatorTok{|}\NormalTok{ID, }\DataTypeTok{correlation=}\KeywordTok{corCompSymm}\NormalTok{(}\DataTypeTok{form=}\OperatorTok{~}\DecValTok{1}\OperatorTok{|}\NormalTok{ID), }
...
\begin{verbatim}
##                 Value Std.Error DF   t-value      p-value
## (Intercept) 10.274104 0.3668354  9 28.007398 4.582957e-10
## XT+          1.253132 0.5103134  7  2.455613 4.374577e-02
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# cannot run t-test with NA so re-run by deleting matching ID}
\NormalTok{fake_data[, Y3}\OperatorTok{:}\ErrorTok{=}\NormalTok{Y2] }\CommentTok{# copy Y2 column}
\NormalTok{fake_data[NA_rows, Y3}\OperatorTok{:}\ErrorTok{=}\OtherTok{NA}\NormalTok{] }\CommentTok{# replace the two obs in Y3 with NA}
\KeywordTok{t.test}\NormalTok{(Y3}\OperatorTok{~}\NormalTok{X, }\DataTypeTok{data=}\NormalTok{fake_data, }\DataTypeTok{paired=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{na.action=}\NormalTok{na.omit)}\OperatorTok{$}\NormalTok{p.value}
\end{Highlighting}
\end{Shaded}
...
A linear mixed model specifically estimates the value of \(\beta_{0j}\)
and \(\beta_{1j}\) for each block

\section{A linear mixed models are
flexible}\label{a-linear-mixed-models-are-flexible}

The linear mixed model above specifies a random effect due to the block
for each term (the intercept and the treatment effect, or ``slope'') in
the model but a researcher might limit the random effect to the
intercept, or less commonly, the slope. Excluding the random slope from
the model above results in
...
We might use random-intercept model if we think that features of the
block would effect the mean response in that block but not effect the
treatment. For example, differences in the immune systems among the
individual mice in experiment 3 might effect growth in both the
wild-type and engineered strains of staph but won't effect the
difference in growth between wild-type and engineered strains

The models above specify a block effect for each term in the model but
the fixed part of the model is relatively simple, there is a single
intercept and a categorical variable with two levels. What if the fixed
part of the model is more complex? A \(2 \times 3\) factorial model has
6 terms: an intercept, a coefficient for the effect of the second level
of factor 1, a coefficient for the effects of each of the non-reference
levels of factor 2, and the two interaction coefficient. The random
block effect can be added to each of these terms

\begin{equation}
y_{ij} = (\beta_{0} + \beta_{0j}) + (\beta_{1} + \beta_{1j}) x_{1i} + (\beta_{2} + \beta_{2j}) x_{2i} + (\beta_{3} + \beta_{3j}) x_{3i} + (\beta_{4} + \beta_{4j}) x_{1i}x_{2i} + (\beta_{5} + \beta_{5j}) x_{1i}x_{3i} + \varepsilon_i
\label{eq:lmm-spec1c}
\end{equation}

\textbf{Not more than you should know} -- Matrix algebra makes the
specification of a linear mixed model much more manageable than the
algebra in \ref{lmm-spec1c}. To use matrix algebra, the fixed and random
terms are arranged together

\begin{equation}
y_{ij} = (\beta_{0} + \beta_{1}x_{1i} + \beta_{2}x_{2i} + \beta_{3}x_{3i} + \beta_{4}x_{1i}x_{2i} + \beta_{5}x_{1i}x_{3i}) + (\beta_{0j} + \beta_{1j}x_{1i} + \beta_{2j}x_{2i} + \beta_{3j}x_{3i} + \beta_{4j}x_{1i}x_{2i} + \beta_{5j}x_{1i}x_{3i}) + \varepsilon_i
\label{eq:lmm-spec1c}
\end{equation}

Then, the fixed terms are \(\boldsymbol{x_i}^\top \boldsymbol{\beta}\)
and the random terms are \(\boldsymbol{x_i}^\top \boldsymbol{\beta_j}\).
Because a linear mixed model may contain

\begin{equation}
\mathbf{y_{ij}} = \mathbf{x_i^\top \beta} + \mathbf{x_i^\top u} + \mathbf{\varepsilon_{ij}}
\end{equation}
...
used function is \texttt{lme()} from the nlme package. The authors of
the lme4 package argue that the df in a linear mixed model are too
approximate for a useful \(p\)-value and, consequently, the \texttt{lme}
function does not return a \(p\)-value. Many biological researchers want
a \(p\)-value and typically use the \texttt{lmerTest} package to get
this.
