...
categorical. A simple example is an experiment with two
\textbf{treatment levels}, such as ``Control'' and ``Knockout''. The
trick is to recode the treatment levels into numbers -- how this is done
is explained in Chapter xxx. When the \(X\) variable is categorical, the
coefficient of the \(X\) is a \emph{difference in means} between the
\(Y\) for a treatment level and the \(Y\) for a reference group
(typically the ``control'' treatment level). The simplest possible model
that can be fit to the data is
