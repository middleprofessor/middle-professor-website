...
The second line of the model (read as ``epsilon is distributed as Normal
with mean zero and standard deviation sigma''``) specifies the
...
model \eqref{eq:lm} is a coefficient) (there are additional parameters
that are hidden from this specification of the linear model but show up
in the specification below). The coefficients of the \(X\) in a linear
...
predictor, which is the \textbf{deterministic} part of the statistical
model.
...
generated. The linear model way ``generates'' data by constructing \(Y\)
from the \(X\) and then adding random, normal error. The statistical
model way ``generates'' data by simply drawing a random variable from a
normal distribution with a specified mean and variance. The statistical
model way explicitly defines all parameters, including the parameters of
the linear predictor (\(\beta_0\) and \(\beta_1\)), the condtional mean
\(\mu\) and the variance \(\sigma\). The linear model way only defines
the parameters of the linear predictor, and often these are referrred to
as ``the parameters'' in the sense that there are not other parameters.
Because of the explicit statement of all parameters, the statistical
model way to define the linear model is more useful in that it is easily
generalized to more complex models, including hierarchical models,
generalized linear models, and Bayesian models. This is why I call it
the ``statistical model'' way of specification.
...
assumptions, to state that there is a 95\% probability that the paramter
lies within the interval. Perhaps a more useful interpretation is that
the interval is a \textbf{compatability interval} in that it contains
the range of estimates that are compatible with the data, in the sense
that a \(t\)-test would not reject the null hypothesis of a difference
between the estimate and any value within the interval (this
...
There are lots or reasons that observations might be correlated. In the
...
\tightlist
\item
  The random draws are from \textbf{identical} probability
  distributions. Identical essentially means that the parameters in the
  probability distribution that control the variance are the same for
  each observation. So, for example, if we model the vole data using the
  normal probability distribution \(N(\mu, \sigma)\) then \(\sigma\) is
  constant for all observations. Indeed, \textbf{constant variance}, or
  \textbf{homoskedasticity}, is an assumption of linear models. If the
  ...
  have variances that are a function of the mean. Heteroskedastic error
  does not imply non-identical probability distributions. For example,
  we could model the number of offspring of voles in the three
  treatments as independent, random draws from a Poisson distribution
  \(Y \sim Poisson(\mu)\). For a Poisson distributed variable, the
  variance equals the mean, so the parameter \(mu\) also controls the
  variance. Let's say the voles in the Control treatment level have, on
  average, twice the number of offspring as the voles in the Vitamin E
  treatment level. The, we'd expect the variance of offspring count in
  the Control group to be twice that than the Vitamin E group. Despite
  the difference variances, these are still draws from the same
  probability distribution. Observations with non-normal distributions,
  such as the Poisson, are modeled using \textbf{Generalized Linear
  Models} (GLM), which are ``extensions'' of the classical linear model.
\end{enumerate}

\section{Specific assumpations of a linear
model}\label{specific-assumpations-of-a-linear-model}

Linear models specifically assume that the probability distribution is
Normal or \textbf{Gaussian}. This implies that 1) the response is
continuous and 2) the probability of sampling an individual measuring
0.5 units below \(\mu\) is the same as the probability of sampling an
individual measuring 0.5 units above the population mean. Counts (number
of cells, number of eggs, number of mRNA transcripts) and binary
responses (sucessful escape or sucessful infestation of host) are not
continous and often often have asymmetric probablity distributions that
are skewed to the right and while sometimes both can be reasonably
modeled using a linear model they are more often modeled using
generalized linear models, which, again, is an extension of the linear
model in equation \eqref{eq:lm-lm-spec2}.
...
As stated above in the ``identical'' assumption, a linear model assumes
that \(\sigma\) is constant (or ``constant error variance'' if we think
about it using the ``linear model'' way of specifying the model). This
assumption is typically violated in both growth and count measures and
while these can sometimes be reasonably modeled using a linear model,
they are often better modeled with a Generalized Linear Model.
Heteroskedastic error arising in other types of biological data can be
modeled with Generalized Least Squares (GLS) models or with linear mixed
models, both of which are simple generalizations of the classical linear
model.
...
Chapter 1 introduced two ways of defining a statistical model.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Definition using a distribution of the error term (the ``linear model
  way'')
\end{enumerate}
...
\item
  ...
  deterministic components (the ``statistical model way'')
\end{enumerate}
...
A generalized linear model has these two parts of the second definition
but adds a third part. Here are the parts of a generalized linear model
...
\item
  ...
  \begin{equation}
  Y \sim \mathrm{Prob}(\mu)
  \end{equation}
\item
  \textbf{the deterministic part}, which is a linear predictor (this is
  more often called the ``systematic part'')
...
Binomial. The link function for the Poisson and Negative Binomial is
\(\eta = log(\mu)\). Note that a linear model is a GLM with a Normal
distribution and an Identity link (\(\eta = \mu\)).

When modeling counts using the Poisson or negative binomial
distributions, the link scale is linear, and so the effects are additive
on the link scale, while the response scale is nonlinear (it is the
exponent of the link scale), and so the effects are multiplicative on
the response scale. If this doesn't make sense now, an example is worked
out below. The inverse of the link function backtransforms the
parameters from the link scale back to the response scale. So, for
example, a prediction on the response sale is
...
Let's fit a GLM with a Poisson link to the worm data. The model is
...
\subsubsection{Model checking -- a quantile residual Q-Q
plot}\label{model-checking-a-quantile-residual-q-q-plot}
...
the residuals of a GLM fit with a Poisson link should be normally
distributed, which could then be checked with a Normal Q-Q plot, and
homoskedastic, which could be checked with a scale-location plot.
Neither of these is true because a GLM does not transform the data and,
in fact, the model definition does not specify anything about the
