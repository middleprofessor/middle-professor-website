...
Diganostic plots for model checking a linear model use the residuals of
the model fit, so the ``linear model'' specification is the natural way
to think about and define the model.

\begin{align}
Diplo_intensity &=\beta_0 + \beta_1 Treatment + \varepsilon\\
\varepsilon \sim N(0, \sigma)
\end{align}
...
Backtransforming these coefficients to the response scale (using
\(\mathrm{exp}{b_j}\)) results in a \textbf{response ratio}.
...
\item
  ...
  response scale is \(RR_{Control}*RR_{Infected\_HG}\) (remember that
  with a linear model the mean would be
