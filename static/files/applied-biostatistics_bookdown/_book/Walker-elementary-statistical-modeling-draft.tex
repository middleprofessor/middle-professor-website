...
Hey that looks pretty good! Compare the observed \(t\) (-1.6) to this
distribution. What is the probability of observing this value given this
distribution?
...
\(p\) is a continuous measure of evidence against the null and it's
value is interpreted subjectively by an informed and knowledgeable
expert using additional information to make decisions. Neyman-Pearson
rejected Fisher's conception of \(p\) as evidence against the null and
used \(p\) as a tool to make a decision that maintains long-term type I
error rates at alpha given a certain power. In Neyman-Pearson, \(p\) is
compared to a threshold, \(\alpha\) and this alone makes the decision.
In Neyman-Pearson, \(p\) is not treated as continuous information.
...
\subsubsection{\texorpdfstring{\(p\) is the probability that the null is
true; \(1-p\) is probability that the alternative is
true}{p is the probability that the null is true; 1-p is probability that the alternative is true}}\label{p-is-the-probability-that-the-null-is-true-1-p-is-probability-that-the-alternative-is-true}
...
\subsubsection{\texorpdfstring{The incredible inconsistency of the
\(p\)-value.}{The incredible inconsistency of the p-value.}}\label{the-incredible-inconsistency-of-the-p-value.}
...
\subsubsection{0.05 is the lifetime false positive
rate}\label{is-the-lifetime-false-positive-rate}
...
concluding an effect exists or ``discovering'' an effect will be 5\%.
This is incorrect. The Type I error rate is the frequency of rejecting
the null when the null is true. This frequency depends on the frequency
of nulls that are true.
...
\item
  40\% are true nulls, this means there are 400 true nulls and 600 true
  effects
\item
  alpha is 5\%. This means we expect to find \(p \le 0.05\) 20 times
  (\(0.05 \times 400\)) when the null is true
\item
  power is 25\%. This means we expect to find \(p \le 0.05\) 150 times
  (\(0.25 \times 600)\) when the null is false
\item
  We have made \(20 + 150=170\) ``discoveries'' (all experiments with
  \(p \le 0.05\)), but
\item
  20 of the 170 or 11.7\% are ``false discoveries''.
\end{enumerate}
...
Problem 3 -- grad students only. Simulate the false discovery rate.
