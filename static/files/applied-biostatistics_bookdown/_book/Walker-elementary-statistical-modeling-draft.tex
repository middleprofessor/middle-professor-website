...
generation of the response is useful for linear models but not useful
for generalized linear models. For example, if we want to model the
number of parasites that have infected a fish using a Poisson
...
That is, we should not think of a count as the sum of some systematic
part and ``random error'', which here is a random draw from a Poisson
distribution. Why? First, if we set the mean of the Poisson ``error'' to
zero (like with a normal distribution), then the error term for every
observation would be zero (because the only way to get a mean of zero
with non-negative integers is if every value is zero). Second, if the
study is modeling the effect of a treatment on the counts (that is, the
\(X\) are dummy variables) then \(\beta_0\) is the expected mean count
of the control (or reference) group. But if we add non-zero Poisson
error to this, then the mean of the control group would be larger than
\(\beta_0\). This doesn't make sense. Finally, this data generation
process generates a continuous response because \(beta_0\) and
\(beta_1\) are continuous (the mean count will typically be something
...
systematic parts but adds a third part.
